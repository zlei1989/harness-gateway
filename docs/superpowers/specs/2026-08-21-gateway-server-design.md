...
| 级别 | 场景 | 行为 |
|------|------|------|
| 帧级 | 单条畸形隧道帧（坏 JSON / 未知 type / 坏数据帧头） | WARN + 丢帧：隧道与在途通道不受影响（与客户端 connection.ts 对称的坏帧预算降级）；被丢帧若属某通道的关键帧，该通道由 headTimeoutMs 兜底 504 清理，不波及其他通道 |
| 请求级 | 无 cookie / 隧道离线 / 等 head 超时 / 探测失败 | 单个浏览器请求 302/401/502/504，不影响其他会话 |
| 通道级 | 客户端回 channel.error / WS 消息超隧道帧上限（边界带护栏，ws-proxy 1009 关本通道，上限契约见客户端 spec §4.2） | 对应浏览器请求 502 / WS 断开 |
| 隧道级 | 隧道断开 / 连续 5 帧畸形（协议版本不匹配） | §5：在途通道失败、tunnels 注销、sessions 保留 |
